...
export const createPullRequest: Tool = {
  ...
  execute: async (args) => {
    ...
    const listResult = await execCommand.execute({
      command: `gh pr list --head ${parsedArgs.head} --base ${parsedArgs.base} --state open --json number`,
    })
    ...
    try {
      const result = await execCommand.execute({
        command: `gh pr create --title ${parsedArgs.title} --body-file ${bodyFile} --base ${parsedArgs.base} --head ${parsedArgs.head}`,
      })
...
export const createIssueComment: Tool = {
  ...
  execute: async (args) => {
    ...
    try {
      await execCommand.execute({
        command: `gh issue comment ${String(parsedArgs.issueNumber)} --body-file ${bodyFile}`,
      })
